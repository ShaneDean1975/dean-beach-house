...
// worker.js
var WUFOO_ENDPOINT = "https://deancamper.wufoo.com/api/v3/forms/sbzzrox09unjge/entries.json";
var ALLOWED_ORIGINS = [
...
var worker_default = {
  async fetch(request, env) {
    ...
    if (!ALLOWED_ORIGINS.includes(origin)) {
      return new Response(
        JSON.stringify({ ok: false, error: "bad origin" }),
        { status: 403, headers }
      );
    }
    let data;
    try {
      ...
      return new Response(
        JSON.stringify({ ok: false, error: "bad body" }),
        { status: 400, headers }
      );
    }
    if (data.website) {
    ...
    if (!body.get("Field1") || !body.get("Field3")) {
      return new Response(
        JSON.stringify({ ok: false, error: "missing name/email" }),
        { status: 400, headers }
      );
    }
    ...
    const detail = out.ErrorText || out.FieldErrors && out.FieldErrors.map((e) => e.ErrorText).join("; ") || "wufoo status " + resp.status;
    return new Response(
